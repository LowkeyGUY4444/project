...
import PopupMsg from './component/PopupMsg'; // Import PopupMsg component
...
const Sign = () => {
  ...
  const signUpWithEmail = async () => {
    try {
      ...
      setOpenModal(true); // Open modal to show error message
    ...
    signInWithEmailAndPassword(auth, email, password)
      .then((userCredential) => {
        if (!userCredential.user.emailVerified) {
          ...
          setOpenModal(true); // Open modal if email is not verified
        } else {
          setHasInteracted(true); // Set interaction state to true
        }
      })
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      >
        ...
        {/* Use PopupMsg component for various messages */}
        <PopupMsg
          open={openModal}
          onClose={() => {
            setOpenModal(false);
